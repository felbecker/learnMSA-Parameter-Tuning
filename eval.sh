...
while read p; do
    filename="${p}.fasta"
    predicted_aln="${run_name}/alignments/$filename"
    projection="${run_name}/alignments/${filename%.fasta}.projection.fasta"
    ...
    col=$(~/bin/t_coffee -other_pg aln_compare -al1 "$reference" -al2 "$projection" -compare_mode column \
                                      | grep -v "seq1" | grep -v '*' | awk '{ print $4}')
    time_file="${run_name}/times/${filename%.ref}.time.txt"
    time="$(grep real $time_file | sed -e "s/^real\t//")"
    echo "${filename%.ref} $sp $modeler $tc $col $time" >> "${run_name}/${run_name}.out"
